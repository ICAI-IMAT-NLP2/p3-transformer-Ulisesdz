...
class Transformer(nn.Module):
    ...
    def __init__(self, src_vocab_size: int, tgt_vocab_size: int, max_enc_position_embeddings: int, max_dec_position_embeddings: int,
        ...
        # The decoder receives tokens of the output lenguage
        ...
        # Receives the output of the decoder
    ...
    def __top_k_sampling_decode(self, src_input: torch.Tensor, max_length: int, k: int = 10, **kwargs) -> torch.Tensor:
        ...
        for _ in range(max_length):
            ...
            log_probs_sort, indx_sort = torch.sort(log_probs, dim=-1, descending=True)
            topk_log_probs, topk_indices = log_probs_sort[:,:k], indx_sort[:,:k]
    ...
    def __contrastive_decode(self, src_input: torch.Tensor, max_length: int, k: int = 5, alpha: float = 0.6, **kwargs) -> torch.Tensor:
        ...
        for _ in range(max_length):
            ...
            # Get the top k tokens
            log_probs_sort, indx_sort = torch.sort(probs, dim=-1, descending=True)
            topk_log_probs, topk_indices = log_probs_sort[:,:k], indx_sort[:,:k]

            # Prepare tensors for all candidates
            seq_len_batch = tgt_input.shape[1]
            expanded_tgt_input = tgt_input.expand(k, seq_len_batch)  # Shape: (k, seq_len)
            ...
            # Hidden state del último token de cada candidato
            h_v = dec_outputs_candidate[:, -1, :]  # Shape: (k, hidden_size)
            # Hidden states de toda la secuencia (excepto el último token)
            ...
            h_v_exp = h_v_norm.unsqueeze(1).expand_as(h_j_norm)  # (k, seq_len, hidden_size)
            cos_sim = F.cosine_similarity(h_v_exp, h_j_norm)  # Shape: (k, seq_len)

            # Get maximum cosine similarity for each candidate
            max_sim = cos_sim.max(dim=1)[0]  # Shape: (k,)

            # Compute scores
            # Probabilidad del modelo para cada candidato
            P_LM_v = topk_log_probs.view(-1)  # Shape: (k,)
